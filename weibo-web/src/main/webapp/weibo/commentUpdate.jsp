...
<SCRIPT LANGUAGE="JavaScript">
	...
	function initCommentData(){
		$.ajax({
			url : '/weibo-studio/selectCommentByKeyWeibo',
			type : 'post',
			async : false,
	...
	function check(){
		...
		$.ajax({
			...
			type : 'post',
			async : false,
			data : {'btvComment.nickname':nickname,'btvComment.location':location,'btvComment.content':content,
				'btvComment.face':face,'btvComment.userid':userid,'btvComment.id':commentId},
			success : function(data){
				var data = data.msg;
				if(data=='success'){
					alert('修改成功!');
			...
			<form method="post">
				...
				<dd>
					<input style="background:url(/weibo-studio/style/webcss/img/chaxunbg.jpg);" 
						id="tj" type="button" onclick="check();" class="submit" value="保存" /> 
					<input style="background:url(/weibo-studio/style/webcss/img/chaxunbg.jpg)" 
