...
<script language="JavaScript">
	...
	function fillWeibo(data) {
		...
		if(data != undefined && data != null && data != '') {
			for(var i=0;i<data.length;i++) {
				var createTime = data[i].taskTime.replace('T',' ');
					...
					html.push("<div class=\"wb_sent\">");
						...
						html.push("<div class=\"date\">");
							html.push("<span class=\"l\">");
								html.push("发布时间:"+createTime);
							html.push("</span>");
							html.push("<span class=\"r la\">");
								html.push("<span class=\"wb_line_l\">|</span>");
								html.push("<a class=\"plLink\" id=\"home_plLink_3712667267535835\" href=\"javascript:del('"+data[i].id+"');\">删除定时</a>");
							html.push("</span>");
						html.push("</div>");
					html.push("</div>");
				html.push("</div>");
	...
	//删除定时任务
	function del(sentId) {
		if (confirm("您确定要删除该定时任务吗?")) {
			var url = '/weibo-studio/deleteTimeSend.action';
			var param = {"bean.id":sentId};
			$.ajax({
				type: "POST",
				url: url,
				data: param,
	...
	function changeTime() {
		...
		window.location = "/weibo-studio/sentPreSend?weibo.weiboId=" + weiboId;
	}
</script>
</head>
...
<body>
	<div class="ipduanguanli" style="overflow:auto;">
		<form id="queryForm" action="/weibo-studio/selectSentSend.action" method="post">
			...
			<input type="hidden" name="example.status" id="status" value="1"/> 
			<dl>
				<dd>
					...
					<input class="button1" value="已发微博" onclick="changeTime();" type="button" />
				</dd>
			</dl>
		</form>
      	<div id="divWrap"></div>
