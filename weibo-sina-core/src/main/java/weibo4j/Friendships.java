...
public class Friendships extends Weibo{
	 ...
	 * @throws WeiboException
	 */
	public JSONObject getAllFollowersById(String uid,String maxTime,long cursorUid) throws WeiboException {
		if(cursorUid!=0){
			return client.get(
					WeiboConfig.getValue("businessUrl") + "friendships/followers/other_all/ids.json",
					new PostParameter[] { new PostParameter("uid", uid),
						new PostParameter("max_time", maxTime),
						new PostParameter("cursor_uid", cursorUid+"")}).asJSONObject();
		}
		return client.get(
				WeiboConfig.getValue("businessUrl") + "friendships/followers/other_all/ids.json",
