	...
	private void constructJson(JSONObject json) throws WeiboException {
		try {
			...
			if(!json.isNull("pic_urls")){
				JSONArray picUrlJsonArray = json.getJSONArray("pic_urls");
				for(int i=0;i<picUrlJsonArray.length();i++){
					picUrls += picUrlJsonArray.optJSONObject(i).getString("thumbnail_pic") + ",";
				}
